...
class SlotBookingController extends Controller
{
    ...
    public function checkAvailability(Request $request)
    {
        ...
        // Validate the expected keys in 'data'
    ...
    public function saveAvailability_old2(Request $request)
    {
        ...
        // Iterate through the slots and add them to the `availability_slots` table
        foreach ($availableSlots as $slot) {
            // Check if the slot already exists using a raw DB query
            ...
            if ($exists) {
                ...
                // Insert the slot and add to new slots
                DB::table('availability_slots')->insert([
                    'tutor_id' => $tutor_id,
                    'slot_start_time' => $slot->slot,
                    'slot_end_time' => $slot->slot_end,
...
// }
    ...
    public function slotscreate(Request $request)
    {
        $request->validate([
            ...
            'classtime' => 'required',
            'endtime' => 'required',
            'repeatOption' => 'nullable|string'
        ]);
    
        $tutorId = session('userid')->id;
    
        $start = Carbon::parse($request->classdate . ' ' . $request->classtime);
        $end = Carbon::parse($request->classdate . ' ' . $request->endtime);
    
        $allCreatedSlots = [];
    
        // Step 1: Create slots for the main day
        $slots = $this->generateHourlySlots($start, $end);
        foreach ($slots as [$slotStart, $slotEnd]) {
            $created = $this->createSlotIfNotExists($tutorId, $slotStart, $slotEnd);
            if ($created) {
                $allCreatedSlots[] = $slotStart->format('Y-m-d H:i');
            }
        }
    
        // Step 2: Repeat if selected
        $repeatOption = $request->input('repeatOption');
        if (in_array($repeatOption, ['forweek', 'formonth'])) {
            $repeatDays = $repeatOption === 'forweek' ? 7 : 30;
    
            for ($i = 1; $i < $repeatDays; $i++) {
                foreach ($slots as [$slotStart, $slotEnd]) {
                    $newStart = $slotStart->copy()->addDays($i);
                    $newEnd = $slotEnd->copy()->addDays($i);
                    $created = $this->createSlotIfNotExists($tutorId, $newStart, $newEnd);
                    if ($created) {
                        $allCreatedSlots[] = $newStart->format('Y-m-d H:i');
                    }
                }
            }
        }
    
        if (count($allCreatedSlots)) {
            return back()->with('success', 'Slots created successfully: ' . implode(', ', $allCreatedSlots));
        } else {
            return back()->with('fail', 'No slots created. Conflicts may exist.');
        }
    }
    
    private function generateHourlySlots(Carbon $start, Carbon $end)
    {
        $slots = [];
    ...
    private function createSlotIfNotExists($tutorId, Carbon $slotStart, Carbon $slotEnd)
    {
        $conflict = SlotBooking::where('tutor_id', $tutorId)
        ...
        if (!$conflict) {
            ...
            $slot->date = $slotStart->format('Y-m-d');
            $slot->slot = $slotStart->format('H:i:s');
            $slot->slot_end = $slotEnd->format('H:i:s');
            $slot->status = 0;
            $slot->class_days = $slotStart->format('Y-m-d');
