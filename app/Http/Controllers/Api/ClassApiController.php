...
class ClassApiController extends Controller
{
    ...
    public function studentclass(Request $request){
        ...
        try{
            ...
            $id = $request->id;
            $classes = SlotBooking::select('slot_bookings.*','tutorregistrations.name as tutor_name')
    ...
    public function studentCompletedclass(Request $request){

        $targetValue = $request->id; // The value we want to check in the JSON array
        ...
        $classes = zoom_classes::select('zoom_classes.*','zoom_classes.id as class_id','zoom_classes.topic_name as topics','zoom_classes.tutor_id as tutor_id','tutorregistrations.name as tutor_name')
                        ...
                        // ->whereRaw("JSON_CONTAINS(batchstudentmappings.student_data, '\"$targetValue\"')")
                        ->where('zoom_classes.is_active',1)
                        ->where('zoom_classes.student_id',$request->id)
                        // ->where('zoom_classes.status','like', '%waiting%')
                        ->where('zoom_classes.is_completed',1)
                        ->paginate(10000);
        $subjects = subjects::where('is_active',1)->get();
        ...
        return response()->json(['message' => 'Subject list', 'subject' => $subjects,'batches'=>$batches,'classes'=>$classes]);
    ...
    public function student_attendance_report(){
        ...
        $attend = studentattendance::select('studentattendances.*','tutorregistrations.name as tutor_name','subjects.name as subject_name')
                                    ...
                                    ->where('studentattendances.student_id', session('userid')->id)->get();
        // dd($attend);
    ...
    public function startclass($id){
        $data = SlotBooking::find($id);
        $data->status = 2;
        $data->class_start_time = Carbon::now('Europe/London');
        $res = $data->update();

        if($res){
            return redirect()->route('tutor.liveclass.classlist')->with('success', 'Class started successfully.');
        }
        else{
            return redirect()->route('tutor.liveclass.classlist')->with('fail','Something went wrong. Failed to start class.');
        }
    }
    public function endclassnow(Request $request){
        $data = SlotBooking::find($request->id);
        $data->status = 3;
        $data->topic = $request->topic;
        $data->ratings_to_student = $request->rating;
        $data->remarks = $request->remarks;
        $data->class_end_time = Carbon::now('Europe/London');
        $res = $data->update();

        if($res){
            return redirect()->route('tutor.liveclass.classlist')->with('success', 'Class completed successfully.');
        }
        else{
            return redirect()->route('tutor.liveclass.classlist')->with('fail','Something went wrong. Failed to end class.');
        }
    }
}
