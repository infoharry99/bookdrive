...
class SubjectsApiController extends Controller
{
    public function index(Request $request){
        ...
        $subjectlist = subjects::select('*','subjects.name as subjectname','subjects.id as subjectid','tutorprofiles.name as tutorname')
                        ...
                        ->where('paymentstudents.student_id',$request->id)
                        ->where('paymentdetails.status',1)
                        ->get();

        return response()->json(['message' => 'Subject list', 'data' => $subjectlist]);
    ...
    public function getsyllabus($id){
        ...
        $topics = topics::select('topics.*', 'syllabi.*','syllabi.name as syllabi_name')
                    ->leftJoin('syllabi', 'topics.id', '=', 'syllabi.topic_id')
                    ->where('topics.subject_id', $id)
                    ->get();
        $syllabus = syllabus::select('*')->get();
        return response()->json(['message' => 'Subject list', 'subject' => $subject,'topics'=>$topics,'syllabus'=>$syllabus]);
