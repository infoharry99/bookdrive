...
class MessagesApiController extends Controller
{
    ...
    // Messages by student
    public function messagesbystudent(Request $request)
    {
        ...
        $id = $request->id;
        $prof_chk = studentprofile::select('*')->where('student_id','=',$id)->count();
        if ($prof_chk == 0) {
            return response()->json([
                'status' => 400,
                'message' => 'Please update your profile details to start messaging. Click on profile > Edit Profile'
            ]);
        }
    ...
    public function messagesbystudenttutormessages(Request $request)
    {
        ...
        $header = tutorregistration::select('tutorregistrations.*', 'tutorprofiles.profile_pic as profile_pic')
                    ->join('tutorprofiles', 'tutorprofiles.tutor_id', 'tutorregistrations.id')
                    ->where('tutorregistrations.is_active', 1)->where('tutorregistrations.id', $tutor_id)
                    ->first();


        $profile_pics = studentprofile::select('profile_pic')->where('student_id', $id)->first();
