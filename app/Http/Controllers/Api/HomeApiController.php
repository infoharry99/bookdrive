...
class HomeApiController extends Controller
{
    ...
    public function tutordetails($id)
    {
        ...
        return view('front-cms.tutordetails', compact('tutorpd','groupedSlots', 'achievement', 'reviews', 'subjects', 'averagecount', 'averagereview', 'totalStudents', 'othertutors', 'primarysubjects'));
    }
    ...
    public function student_login(Request $request)
    {
        ...
        if ($request->loginAs == 'student') {$user = studentregistration::where('email', '=', $request->username)->first();
            if ($user) {
                if (Hash::check($request->password, $user->password)) {
                    //  event(new Registered($user));
        ...
        if ($request->loginAs == 'tutor') {
            $user = tutorregistration::where('email', '=', $request->username)->first();
            ...
            $this->validate(request(), [
                ...
                $notifications = Notification::orderBy('created_at', 'desc')
                    ...
                    ->where(function ($query) {
                        $query->where('show_to_student_id', $id)
    ...
    function makearequest(Request $request){
        ...
        $request->validate([
            ...
            'noOfClasses' => 'required|integer|min:1',
            'startDate' => 'required|date',
        ]);
    
        // Create a new instance of TutorRequest model
        $data = new TutorRequest();
    
        // Map the request data to the model's fields
        $data->full_name = $request->fullName;
        $data->mobile = $request->mobile;
        $data->email = $request->email;
        $data->city = $request->city;
        $data->location = $request->location;
        $data->no_of_classes = $request->noOfClasses;
        ...
        if($res){
            ...
            return back()->with('success', 'Request submitted successfully. Please allow us some time to review your request, and we will inform you.');
        } else {
            // If something went wrong, return back with an error message
            return back()->with('error', 'There was an issue submitting your request. Please try again.');
        }
    }
    
     public function storePostcodeAndMobile(Request $request)
    {
        // Validate the request
        $validated = $request->validate([
            'postcode' => 'required|string|max:255',
            'mnumber' => 'required|numeric',
            'opt_in' => 'nullable|boolean',  // Optional field for opt-in checkbox
        ]);
        
         $optInValue = $request->has('opt_in') ? true : false;

        $storePostdataId = DB::table('store_postdata')->insertGetId([
                'postcode' => $postcode,
                'mobile' => $mobile,
                'opt_in' => $opt_in,
                'created_at' => now(),
        ...
        // Redirect to the advance search page
    ...
    public function show()
    {
     ...
     public function storeLessonPreferences (Request $request)
    {
       
         $postcode = $request->input('postcode');
    $mobile = $request->input('mnumber');
    $opt_in = $request->has('opt_in') ? true : false; // Check if the opt-in checkbox is checked

       // print_r($request['lessonstype']);exit;
    ...
    public function storehomes(Request $request)
{

   // dd($request['lessonstype']);
//     $validated = $request->validate([
//             'lessonstype' => 'required|string',
//             'transmission' => 'required|string',
//             'fasttrack' => 'required|string',
//             'spreadoutLesson' => 'nullable|string',
//             'flexible' => 'required|string',
//             'lessions' => 'required|string',
//         ]);

//   dump($validated);
    
    // Insert into lesson_preferences table using the inserted store_postdata_id
    DB::table('lesson_preferences')->insert([
        'lessonstype' => $request['lessonstype'],
            'transmission' => $request['transmission'],
            'fasttrack_theory' => $request['fasttrack'],
            'ultimate_theory_package' => $request['spreadoutLesson'] ?? null,
            'fasttrack_driving' => $request['fasttrackdriving'],
            'booked_driving_test' => $request['bookedDrivingtest'],
            'driving_test_date' => $request['driving_test_date'] ?? null,
            'flexible' => $request['lesson_preference'],
            'previous_driving_experience' => $request['previous_driving_experience'] ?? null,
...
public function checkPostcode($postcode)
    {
        // Get the postcode prefix (first part before any space)
        $postcodePrefix = strtolower(explode(' ', $postcode)[0]);

        // Check if the postcode prefix exists in the 'subjects' table
        $exists = Subject::where('name', $postcodePrefix)->exists();

        // If exists, return 'valid', else return 'invalid'
        return response()->json($exists ? 'valid' : 'invalid');
    }
}
