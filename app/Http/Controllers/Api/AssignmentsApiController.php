...
class AssignmentsApiController extends Controller
{
    ...
    public function studentassignmentslist(Request $request)
    {
        ...
        $assignments = StudentAssignmentList::select('*', 'student_assignment_lists.id as assignment_id', 'tutorregistrations.name as tutor_name', 'student_assignment_lists.topic', 'student_assignment_lists.name as assignment_name', 'subjects.name as subject', 'classes.name as class',)
                        ...
                        ->join('tutorregistrations', 'tutorregistrations.id', 'student_assignment_lists.assigned_by')
                        ->where('student_assignment_lists.is_active', 1)
                        ->where('student_assignment_lists.student_id', $id)
                        ->orderBy('student_assignment_lists.created_at', 'desc')
                        ->paginate(10);

            $submissions = StudentAssignments::select('*')->where('submitted_by',$id)->where('is_active',1)->get();
            $res = [
                'status' => 'success',
                'data' => [
    ...
    public function referral(){
        return view('tutor.referral');
    }

    public function theorytestschedule(){
        return view('tutor.theorytestschedule');
    }

    public function practicaltestschedule(){
        return view('tutor.practicaltestschedule');
    }
